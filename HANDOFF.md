...
**Eighteen items, (a)…(r)**, counted 2026-09-02. **(g)/(h) CLOSED**, **(e)/(l) SPENT** (see
...
`NotLeafName`-shaped (a `relNameOK` one RE-VACUATES the headlines); (q) the grep OVER-counts this cone, size it with the probe; (r) the six W3d endpoints are audited but NOT statement-pinned, so adding a premise is invisible to the gate. (p): it is the SCHEMA
