...
> 2026-09-13 — **`P6` step 0 LANDED, and it corrected the reason 2026-09-12b gave it.** Wall 1 is dissolved but NOT by `W4Fragment.term`: the probe store `Sd` **satisfies** `term` (`htermB Sd Td = true`, measured), because `NoTtuTarget` quantifies over `schemaRewrites`, which DROPS DERIVED DEFS, and the arm's owner key is tainted too via `exprRefs`'s `.ttu` case. What holds is stronger — `schemaRewrites Sd = []`, so narrow AND widened predicates are **VACUOUS** there (not engaged), and `Sd` fails `computedOrDirect` for EVERY store. Had the pin been written as 2026-09-12b specified it would have reddened on first build. Landed additive: `TtuStarWide.lean::ttuStarFreeW_through_untainted` + 14 `decide` pins (`::Zanzibar.RoutingArmWitness`, `::Zanzibar.TermNonvacuityWitness`) + the sweep table + `CORRESPONDENCE.md` §7.3 (612 anchors, all resolve). `TtuStarFreeW` untouched, zero audited names re-opened. **NEW open question:** `term`'s `NoTtuTarget` half may be IMPLIED by `RewriteMatchDeclared` inside the chain — unmeasured, on the row.
...
> → **`P6` is `NOW` and startable at step 1** — the zero-cone go/no-go probe on the IN-FRAGMENT store (`WideWitness.SwT` is that shape). Do NOT narrow `TtuStarFreeW`; do NOT touch `W4Fragment.ttuStarFree` before step 4. ⚠ **Step 1's probe must assert PER-ARM NON-VACUITY as well as reporting PER DOMAIN**: an arm measured where `schemaRewrites = []` measures nothing, which is exactly how the 2026-09-12 ROUTING arm produced a number that meant nothing, and arm B-SUB-L's shrunken domain is the `hunmapped` premise working rather than vacuity. **Decide Lean-shaped questions yourself** — `CLAUDE.md` § "Who decides": the user delegates architecture, the goal is graph index output EQUAL to the set engine's, a divergence means fix the Python; if stuck, a `fable` subagent makes the call. `R6` is `NEXT` and startable; NOT parallel-safe with `P6` step 3. `TK57`–`TK65` and `TK53` are DONE; `TK62` stays DECLINED.
