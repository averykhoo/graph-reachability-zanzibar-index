...
> ✅ **`2026-09-01d`: §11.13 trap (g) is dissolved** — both `hag`s in
> `ReconcileStars.lean::checkFn_agree_of_graphRec{,_cd}` carry `r' ∈ computedRefs e` now,
> consumed by `CascadeStable.lean::checkFn_agree_of_graphRec_notLeafNode`. **The `hnl`
> binder is still deferred, but by step 9 rather than the trap**: 3 of the 14 sites pass the
> query's own relation. Sabotage, traps and the `3 + 8 + 3` partition: scope doc §11.13 (g).
> ✅ `2026-09-01b`: Class-B repair landed, all four conditions discharged; the `hql` surface
> is one row again by MIGRATION (rotated 2026-09-01d → PROOF_STATUS `2026-09-01b`, row `P3`).
> ✅ **`2026-09-01c`: step 7's predicate landed as `ComputedRefsNotLeaf`** — its named
> premise was false (Python dot-locks referenced names, it does not require declaredness),
> 8 pins + 2 sabotages, inert so those pins are the *sole* evidence. **"Step 7" is
> ambiguous — three numberings are live and this board mixed them: `hql` is step 10, and
> step 9 (the flip) is what it co-lands with.** `shadow_graphRec_agree` has **14** sites.
> ✅ Step 6: `hv3` pre-widened, zero new premises, green; **4 obligations left, not 5**; new
> §11.13 **(k)**: a sabotage can fail on a *redundant* guard.
