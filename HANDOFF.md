...
> ✅ **`2026-09-02b`: the remaining 4c-ii co-landing is SIZED BY PROBE and is a CLOSED LIST
> — 14 sites / 12 declarations / 6 files**, and `hql`'s eliminator
> `Leaf.lean::not_leafNode_of_publicOfLeaf_none` LANDED (additive; `lean` identical to
> anchor, 585/584/164). `graph_correct_w3d2` and `_d` each host TWO sites, so the two repairs
> are not separable at declaration granularity. **NEXT is the atomic co-landing; nothing
> smaller is green-stoppable.** Detail + the (t)/(u) traps and the (r) correction:
> session-log `2026-09-02b` → PROOF_STATUS `## Session 2026-09-02b`.
> ✅ **`2026-09-02`: step 9 is done** — threaded at all three `hsubj` sites and discharged
> from `GraphAdmission`, so the headline theorems assume exactly the bundle they did before.
> Definition pin regenerated deliberately 161→164; statement pin byte-identical. Residual:
> PROOF_STATUS §7 — milestone theorems carry extra spelled-out premises, gate-invisible (its
> "six" is corrected to ~38 by `2026-09-02b`; both unverified).
> ✅ `2026-09-01b`/`c`/`d`, `2026-08-31b` rotated → PROOF_STATUS. Live from them: **"step 7"
> is ambiguous — three numberings, `hql` is step 10**; §11.13 (k) is generalised by (n).
...
pre-widen. **Steps 1–9 have ALL landed (corrected 2026-09-02b — this line used to read "the
flip is step 9 and is a two-line rollback", contradicting this block's own tail); what is
left is the ATOMIC co-landing.** Per step: PROOF_STATUS `2026-08-30b`…`2026-09-02b`.
...
**Nineteen items, (a)…(s)**, counted 2026-09-02. **(g)/(h) CLOSED**, **(e)/(l) SPENT** (see
...
`NotLeafName`-shaped (a `relNameOK` one RE-VACUATES the headlines); (q) the grep OVER-counts this cone, size it with the probe; (r) the six W3d endpoints are audited but NOT statement-pinned, so adding a premise is invisible to the gate; (s) step 9 is DONE and the flip is down to `hv1` alone. (p): it is the SCHEMA
