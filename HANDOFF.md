...
> ✅ **`2026-09-01e`: step 8's free half landed; the flip is now a closed list** — three
> `apply*` lemmas pre-widened, and **the tree builds green under the flip with only FOUR
> obligations `sorry`-stubbed**, retiring "~20 sites in 3 files" for **5 sites / 4 decls / 2
> files**. **Step 9 is blocked on a DESIGN DECISION** — `P3` row + new scope-doc trap **(n)**.
...
✅ **Step 8's free half landed 2026-09-01e, and the flip is measured end to end.** Three
`apply*` lemmas pre-widened in place — no new declaration, no signature change, no pin
exposure; `hoffW` costs no premise (`hcb` + `bare_subjNode_not_leafNode`). Staging the probe
with `sorry` on each already-blocked obligation builds past the red module: **the flip is 5
sites / 4 decls / 2 files**, and with only the four unowned obligations stubbed the whole
tree is green. It sizes *structure*; it does not license landing the flip.

🚨 **Step 9 is blocked on a design decision, not proof effort — carry this forward.** Free:
delete `shadow_graphRec_agree`'s two `Or.inl` wrappers (`:1412-1413`). Else: `hv1` needs the
operand-side `NotLeafName r'` (do not bundle with `hv3`), and the seed-side `NotLeafName
t.subject.predicate` — shared by `reachedByW3d_shadow` (`:1332`), `reachedByW3d2_shadow`
(`:720`), `::_d` (`:1319`) — wants a **store-level** `NoLeafStoreSubjects T` threaded through
their signatures, changing downstream statements and hence potentially the headline theorems.
**That is the human call this item flags. Do not thread it unasked.** `2026-09-01e` + (n).
...
**Fourteen items, (a)…(n)**, counted 2026-09-01e — the "Ten" this line carried was stale by
four. **(g)/(h) CLOSED**, **(e)/(l) SPENT** (see **(m)**), **(b) itself stale** (cite
`file::symbol`), **(n) new** — it retires "~20 sites in 3 files" and generalises **(k)**: a
PRE-WIDEN's control is the flip PROBE, never a weakening. **Exit: §11.12** via its
