...
> ✅ **`2026-09-02`: step 9 is done** — design call made (user: thread and discharge), all
> three `hsubj` sites threaded, and `GraphAdmission` carries the two new fields, so the
> headline theorems assume exactly the bundle they did before. Definition pin regenerated
> deliberately 161→164; statement pin byte-identical. 📏 **The flip is now one obligation**
> (`hv1`); the rest is eight mechanical `Or.inl` deletions, and with `hv1` alone stubbed the
> whole tree builds — `2026-09-01e`'s "four" is retired. Residual: PROOF_STATUS §7 — six
> milestone theorems carry two extra spelled-out premises, gate-invisible.
> ✅ `2026-09-01d`: §11.13 trap (g) dissolved, the `hnl` binder deferred to step 9 (rotated
> 2026-09-01e → PROOF_STATUS `2026-09-01d`, row `P3`).
> ✅ `2026-09-01b`/`c`, step 6 (rotated → PROOF_STATUS `2026-09-01b`/`c`, `2026-08-31c`).
> Live from them: **"step 7" is ambiguous — three numberings, `hql` is step 10**;
> `shadow_graphRec_agree` has **14** sites; §11.13 **(k)** is generalised by (n).
