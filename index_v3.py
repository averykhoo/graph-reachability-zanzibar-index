...
def _add_db_edges_unsafe(subject_id: int | None,
                         object_id: int | None,
...
def _add_edge_unsafe(subject_id: int | None,
                     object_id: int | None,
                     multiplier: int | None,
                     ):
    ...
    if subject_id != object_id and multiplier < 0:
        _add_db_edges_unsafe(subject_id, object_id, multiplier, multiplier)
    else:
    ...
    with Session(engine) as session:
        ...
        for triple in triples_from:
            reachable_from_node_from[triple.subject_id] = triple.indirect_edge_count
        triples_to = session.exec(select(Edge)
                                  .where(Edge.subject_id == object_id)
                                  ).all()
        for triple in triples_to:
            reachable_from_node_to[triple.object_id] = triple.indirect_edge_count

    # ensure we're not creating a cycle
    assert reachable_from_node_from[object_id] == 0, reachable_from_node_from
    assert reachable_from_node_to[subject_id] == 0, reachable_from_node_to
    ...
    for (_subject_predicate, _subject_id), from_count in reachable_from_node_from.items():
        for (_relation, _object_id), to_count in reachable_from_node_to.items():
            _add_edge_unsafe(_subject_id, _object_id, from_count * to_count * multiplier)

    # add the node_to's reachable nodes to node_from
    for (_subject_id, _relation), count in reachable_from_node_to.items():
        _add_edge_unsafe(subject_id, _subject_id, count * multiplier)
    ...
    for (_subject_predicate, _subject_id), count in reachable_from_node_from.items():
        _add_edge_unsafe(_subject_id, object_id, count * multiplier)
...
def add_edge(subject_id, object_id):
    ...
    _add_edge_unsafe(subject_id, object_id, 1)
