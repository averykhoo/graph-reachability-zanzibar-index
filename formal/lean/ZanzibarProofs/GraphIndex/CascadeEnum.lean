...
/-- **The star-free analog of `reachedByW3d_Rnode_source_bare`.** Every in-edge source
    at a derived R-node is star-free on a W3d state: write legs never
...
theorem reachedByW3d_Rnode_source_name_ne_star {σ : GraphState} {S : Schema} {T : Store}
  ...
  -- `WF S` is new (step 4c-ii), for the same reason as in `reachedByW3d_Rnode_source_bare`:
  -- the write case's `writeLeg_derived_inedges_eq` needs it post-flip.
  induction h with
  | empty S =>
...
theorem mem_affectedKeys_props {S : Schema} {σ : GraphState} {d : Delta}
  ...
  · -- LeafFamily own-key branch, **(alpha)**: `k = (d.node.type, R, d.node.name)` where
    -- `publicOfLeaf S d.node.type d.node.pred = some R`. `isDerived` comes from
    -- `Leaf.lean::isDerived_of_publicOfLeaf` (it used to be read straight off the guard's
    -- now-deleted third conjunct); `k.2.2 ≠ STAR` still comes from `hc`.
    by_cases hc : d.leaf = true ∧ d.node.name ≠ STAR
    · rw [if_pos hc] at hown
      cases hpl : publicOfLeaf S d.node.type d.node.pred with
      | none => rw [hpl] at hown; exact absurd hown List.not_mem_nil
      | some R =>
        rw [hpl] at hown
        obtain rfl := List.mem_singleton.mp hown
        obtain ⟨_, hname⟩ := hc
        have hder := (isDerived_of_publicOfLeaf hpl).1
...
theorem w3cJobValid_enumJob {S : Schema} {T : Store} {σ : GraphState}
  ...
  · -- cands are bare
    ...
    · rw [edgeHolders, List.mem_map] at hcr
      ...
      rw [← hce]
      exact reachedByW3d_Rnode_source_bare h hWF hlk hder hco hSV ed.1 hmem
  · -- cands are star-free
    intro c hc
    simp only [enumJob, List.mem_append] at hc
    rcases hc with hcl | hcr
    ...
    · rw [edgeHolders, List.mem_map] at hcr
      ...
      have hmem : (ed.1, objNode ⟨dt, on⟩ R) ∈ σ.edges := by rw [← hb]; exact hedm
      rw [← hce]
      exact reachedByW3d_Rnode_source_name_ne_star h hWF hlk hder hco hSV ed.1 hmem
...
theorem enumJobs_valid {S : Schema} {T : Store} {σ : GraphState}
  ...
  rw [← hje]
  exact w3cJobValid_enumJob hWF hNK hSV hCO h hlk hder hon
...
theorem enumJobs_covg {S : Schema} {T : Store} {σ : GraphState}
  ...
  obtain ⟨hder, _, hon⟩ := mem_cascadeKeys_props hk
  rw [← hje]
  exact w3dJobCoverage_enumJob hWF hTT hNK hR hSV hBS hTS hCO hMatch hStrat hQ hDR hLS hcr hNBD hterm h
...
theorem reachedByW3dE_toC {σ : GraphState} {S : Schema} {T : Store}
  ...
  | @cascade σp S T hprev ih =>
    ...
    exact ReachedByW3dC.cascade (enumJobs S σp)
      ...
      (enumJobs_covg hWF hTT hNK hR hSV hBS hTS hMatch hStrat hQ hDR hLS hcr hNBD hterm hCO hLU
        hWSbare hW3d)
      hC
...
theorem graph_correct_w3dE {S : Schema} {T : Store} {σ : GraphState} (q : Query)
  ...
  graph_correct_w3d q hWF hTT hNK hR hSV hBS hTS hMatch hStrat hQ hDR hLS hcr hNBD hterm hCO hLU
    hWSbare
    (reachedByW3dE_toC h hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU hWSbare hSV hBS hTS
      hterm)
    hq hqs hqo hql
...
theorem reachedByW3dE_inv {σ : GraphState} {S : Schema} {T : Store}
  ...
  reachedByW3dC_inv
    (reachedByW3dE_toC h hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU hWSbare hSV hBS hTS
      hterm)
    hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hCO hLU hWSbare hSV hBS hTS hNBD hterm
