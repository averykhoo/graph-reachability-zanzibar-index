...
theorem reconcileJobsLR_key_edge_sem {S : Schema} {T : Store} {σ0 : GraphState}
  ...
  | cons j rest ih =>
    ...
    have hsh' : UntaintedShadow S (j.applyLoggedR S T σ) σ0 :=
      untaintedShadow_applyLoggedR hsh (reachedByRules_of_admitted h0) hSV hNK hCO hjv1
    ...
    · by_cases hkm : j.keyMatch dt on R
      · -- the pass targets the key: the routed char at pass start + the bridge
        ...
        · rw [Bool.and_eq_true] at hguard
          ...
          rw [checkFnR_eq_sem_settled hWF hTT hNK hR hSV hBS hTS hMatch hStrat
            hterm hCO hWSbare hcr h0 hsh hσS hlk hder hco hLU2e hopsB
...
theorem settledComplete_jobsLR_targeted {S : Schema} {T : Store} {σ σ0 : GraphState}
  ...
  have hbridge : ∀ (x : SubjectRef), (x.name = STAR → x.predicate = BARE) →
    ...
    fun x hx => checkFnR_eq_sem_settled hWF hTT hNK hR hSV hBS hTS hMatch hStrat
      hterm hCO hWSbare hcr h0 hshpre hσpreS hlk hder hco hLU2e hopsB hx hon
  have hcovsem : ∀ sh ∈ declaredWildcardShapes S,
      σpre.coveredFnR T dt on R e sh = sem S T ⟨starSubj sh, R, ⟨dt, on⟩⟩ :=
    fun sh hws => hbridge (starSubj sh) (fun _ => hWSbare sh hws)
  -- Since the 2026-09-14h split the candidate list is Python's two-pass one, so a row
  -- member need not be declared on its face; the ROUTED linchpin recovers it from coverage.
  have hdeclOfCov : ∀ sh : Shape, σpre.coveredFnR T dt on R e sh = true →
      sh ∈ declaredWildcardShapes S := fun _ hcov =>
    checkFnR_star_declared hTT hSV hTS h0 hshpre hσpreS hcr hlk hco hon hopsB hcov
  ...
  have hstars_iff : ∀ sh : Shape,
      ((wildcardShapes S).filter (fun sh => σpre.coveredFnR T dt on R e sh)).contains sh
          = true
        ↔ (sh ∈ declaredWildcardShapes S ∧ sem S T ⟨starSubj sh, R, ⟨dt, on⟩⟩ = true) := by
    intro sh
    ...
    · rintro ⟨hws, hsm⟩
      refine decide_eq_true
        (List.mem_filter.mpr ⟨mem_wildcardShapes_of_mem_declared hws, ?_⟩)
      rw [hcovsem sh hws]
      exact hsm
  constructor
  · -- === the settled half ===
    ...
    · -- every derived edge witnesses a `sem`-true subject
      ...
      · rw [Bool.and_eq_true] at hguard
        ...
        exact hchk
      · rcases reconcileJobsLR_key_edge_sem hWF hTT hNK hR hSV hBS hTS hMatch
            hStrat hterm hcr hNBD hCO hWSbare h0 hlk hder hRne hon hLU2e pre σ hσS hjvpre hsh
...
theorem settledComplete_cascade2_targeted {σ : GraphState} {S : Schema} {T : Store}
  ...
  obtain ⟨σ0, h0, hsh⟩ := reachedByW3d2_shadow h hNK hCO hSV hterm hQ hDR hLS hMatch hBS hNBD hTT hTS
  ...
  have hRns : ∀ y, (objNode ⟨dt, on⟩ R, y) ∉ σ.edges :=
    reachedByW3d2_Rnode_not_source hterm hRne hNBD hder h
  have hsbOps : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ∀ x, (x, objNode ⟨dt, on⟩ r') ∈ σ.edges → x.pred = BARE := by
    intro r' hr' hd' x hx
    obtain ⟨e', hlk'⟩ := isDerived_declared hd'
    have hco' : ComputedOnly e' := hCO dt r' e' hlk' hd'
    exact reachedByW3d2_Rnode_source_bare h hWF hlk' hd' hco' hSV x hx
  -- the accept form (the reject branch is dead at two strata)
  have hacc := runCascade2_no_abort hterm hNBD hLU2 hjv1 hjv2 hscope2 h
  ...
  have hfence : ∀ j2 ∈ jobs2, ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
    ...
    rw [hkey2] at hjk
    obtain ⟨r'', hr''mem, hd''⟩ := round2_key_reads_derived hterm hNBD hjv1 h hlk' hjk
  ...
  · -- ==== Case A: settle over round 2 from the MID state ====
    ...
    have hopsMid : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      · -- targeted in round 1 ⇒ re-settled by the batch lemma at σ (hops vacuous)
        ...
        have hRne' : r' ≠ BARE := by
          ...
          exact (hjv1 j1 hj1).1
        have hRns' : ∀ y, (objNode ⟨dt, on⟩ r', y) ∉ σ.edges :=
          reachedByW3d2_Rnode_not_source hterm hRne' hNBD hd' h
        ...
        refine settledComplete_jobsLR_targeted hWF hTT hNK hR hSV hBS hTS hMatch
          hStrat hterm hcr hNBD hCO hWSbare h0 hsh hσS hjv1
      ...
      · -- untargeted in round 1 ⇒ clean at leg start, settled at σ, transported
        ...
        · exfalso
          obtain ⟨j1, hj1, hkey1⟩ := hcover1 _ ((mem_cascadeKeys_iff_above _ _ _).mp hdirty)
    ...
    have hsettled := settledComplete_jobsLR_targeted hWF hTT hNK hR hSV hBS hTS
      hMatch hStrat hterm hcr hNBD hCO hWSbare h0 hshmid hσmidS hjv2
  ...
  · -- ==== Case B: the last targeting job is in round 1; round 2 is inert here ====
    ...
    have hopsS : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      · exact hsc
    have hsettled1 := settledComplete_jobsLR_targeted hWF hTT hNK hR hSV hBS hTS
      hMatch hStrat hterm hcr hNBD hCO hWSbare h0 hsh hσS hjv1
...
theorem sem_nil_derived_false2 {S : Schema}
  ...
  obtain ⟨σ0, h0, hsh⟩ := reachedByW3d2_shadow hchain hNK hCO hSV hterm hQ hDR hLS hMatch hBS hNBD hTT hTS
  ...
  have hops : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
    ...
    have hsemF' : ∀ (x : SubjectRef), (x.name = STAR → x.predicate = BARE) →
      ...
      fun x hx => sem_nil_derived_false hWF hTT hNK hR hCO hMatch hStrat htermS
        hlk' hco' hlu' hx hon
  ...
  rw [← checkFnR_eq_sem_settled hWF hTT hNK hR hSV hBS hTS hMatch hStrat hterm
    hCO hWSbare hcr h0 hsh hσS hlk hder hco hLU2e hops hs hon]
...
theorem reachedByW3d2C_settled {σ : GraphState} {S : Schema} {T : Store}
  ...
  induction h with
  | empty S =>
    intro hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2 hWSbare _hSV _hBS _hTS hterm
      dt on R e hlk hder hon
    have hsemF : ∀ (x : SubjectRef), (x.name = STAR → x.predicate = BARE) →
        sem S [] ⟨x, R, ⟨dt, on⟩⟩ = false :=
      fun x hx => sem_nil_derived_false2 hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD
  ...
  | @write σp S T t hadm hprev ih =>
    ...
    rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2 hWSbare hSVw hBSw hTSw htermw
    ...
    · -- settled at σp, key and every derived operand key unmapped: `sem` stable at
      ...
      have hopsSettled : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
        ...
        intro r' hr' hd'
        obtain ⟨e', hlk'⟩ := isDerived_declared hd'
        rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2 hWSbare hSVw hBSw hTSw
      ...
      have hsem : ∀ x : SubjectRef, (x.name = STAR → x.predicate = BARE) →
        ...
        fun x hx => writeLeg_sem_stable2 hWF hTT hNK hR hSV hBS hTS hMatch hStrat hQ hDR hLS hcr
          hNBD hterm hCO hLU2 hWSbare hW3d2 hadm hlk hder hmap hopsUnmapped hopsSettled hx hon
      exact Or.inr (Or.inr
        ⟨settledKey_writeLeg_sem hWF hNK hSV hCO hWSbare hlk hder hsem hset,
          completeKey_writeLeg_sem hWF hNK hSV hCO hWSbare hlk hder hsem hcomp⟩)
  | @remove σp S T t hadm hdrain hSVT hBST hTST htermT hprev ih =>
    ...
    have hW3d2 : ReachedByW3d2 σp S T := reachedByW3d2C_toW3d2 hprev
    rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2 hWSbare hSVT hBST hTST htermT
        dt on R e hlk hder hon with hdirty | hopdirty | ⟨hset, hcomp⟩
    ...
    · have hopsSettled : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      have hopsUnmapped : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
        ...
        fun r' hr' hd' hmem => hopmap ⟨r', hr', hd', hmem⟩
      obtain ⟨σ0, h0, hsh⟩ :=
        reachedByW3d2_shadow hW3d2 hNK hCO hSVT htermT hQ hDR hLS hMatch hBST hNBD hTT hTST
      obtain ⟨σ0', h0', hsub⟩ := exists_admitted_erase h0 t
      have hsem : ∀ x : SubjectRef, (x.name = STAR → x.predicate = BARE) →
          sem S (T.erase t) ⟨x, R, ⟨dt, on⟩⟩ = sem S T ⟨x, R, ⟨dt, on⟩⟩ :=
        fun x hx => removeLeg_sem_stable2 hWF hTT hNK hR hLS hSVT hBST hTST hMatch hStrat
          htermT hcr hCO hLU2 hWSbare hW3d2 hadm h0 hsh h0' hsub hlk hder hmap hopsUnmapped
          hopsSettled hx hon
      exact Or.inr (Or.inr
        ⟨settledKey_removeLeg_sem hWF hNK hSVT hadm hCO hWSbare hlk hder hsem hset,
          completeKey_removeLeg_sem hWF hNK hSVT hadm hCO hWSbare hlk hder hsem hcomp⟩)
  | @cascade σp S T jobs1 jobs2 hjv1 hjv2 hcover1 hscope1 hcover2 hscope2 hcovg1 hcovg2
    ...
    · -- targeted ⇒ re-settled by the two-round settle theorem
      have hopsBase : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
        ...
        intro r' hr' hd'
        obtain ⟨e', hlk'⟩ := isDerived_declared hd'
        rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2 hWSbare hSV hBS hTS hterm
            dt on r' e' hlk' hd' hon with hdirty' | hopdirty' | hsc
        · exact Or.inl hdirty'
        · obtain ⟨r'', hr'', hd'', _⟩ := hopdirty'
          cases (hLU2 dt R e hlk hder r' hr' hd' e' hlk' r'' hr'').symm.trans hd''
      ...
      exact Or.inr (Or.inr (settledComplete_cascade2_targeted hWF hTT hNK hR hSV hBS hTS
        hMatch hStrat hQ hDR hLS hcr hNBD hterm hCO hLU2 hWSbare hW3d2 hjv1 hjv2 hcover1 hcover2
    ...
    · -- untargeted: both dirty disjuncts force a targeting job; settled transports
      ...
      rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2 hWSbare hSV hBS hTS hterm
          dt on R e hlk hder hon with hdirty | hopdirty | ⟨hset, hcomp⟩
      · exfalso
        obtain ⟨j, hj, hkey⟩ := hcover1 _ ((mem_cascadeKeys_iff_above _ _ _).mp hdirty)
      ...
      · have hacc := runCascade2_no_abort hterm hNBD hLU2 hjv1 hjv2 hscope2 hW3d2
...
theorem graph_correct_w3d2 {S : Schema} {T : Store} {σ : GraphState} (q : Query)
  ...
  obtain ⟨σ0, h0, hsh⟩ :=
    reachedByW3d2_shadow hW3d2 hNK hCO hSV hterm hQ hDR hLS hMatch hBS hNBD hTT hTS
  obtain ⟨⟨st, sn, sp⟩, R, ⟨dt, on⟩⟩ := q
  replace hqs : sn = STAR → sp = BARE := hqs
  replace hqo : on ≠ STAR := hqo
  -- `obtain` cased on `q`, so the query guard returns in projection form.
  ...
  · -- ===== derived query: the settled-key read =====
    ...
    have hsettledAt : ∀ dt₀ on₀ R₀ e₀, S.lookup (dt₀, R₀) = some e₀ →
      ...
      rcases reachedByW3d2C_settled h hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCO hLU2
    ...
    have hops : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      exact ⟨hset', hcomp',
        fun u hu => reachedByW3d2_reach_collapse_root hWF hSV hlk' hd' hco' hW3d2 hu⟩
    ...
    have hsem_ws : ∀ sh : Shape, sh.2 = BARE →
      ...
      have hchk : σ.checkFnR T (starSubj sh) dt on R e = true := by
        rw [checkFnR_eq_sem_settled hWF hTT hNK hR hSV hBS hTS hMatch hStrat
          hterm hCO hWSbare hcr h0 hsh hschema hlk hder hco hLU2e hops
          (fun _ => hshb) hqo]
        exact hsm
    ...
    exact probeDerived_eq_sem_settled hWSbare hcl
      (fun u hu => reachedByW3d2_reach_collapse_root hWF hSV hlk hder hco hW3d2 hu)
...
/-! ## The Direct-arm (`_d`) settledness chain (leg 5d, steps 2–3)

The `_d` clones of the batch/cascade re-settlement family under `StoreValidRulesD`
+ schema-wide `ComputedOrDirect ∧ DirectArmsBare` (derived OPERAND defs
`ComputedOnly` — `hCOop`, the landed `_filt` machinery's scope). The only content
changes vs the `ComputedOnly` originals: the guard fold-invariance goes through the
CD tree congruence (`evalE_computedOrDirect` — a bare `Direct` arm reads the FIXED
store, inert under the pass), the shadow is the FILTERED-σ0 pair
(`reachedByW3d2_shadow_d` + the `_d` shadow transports), and the structural chain
facts are their `_d` forms (own-key dirtiness / bare seed subjects). -/

/-- The routed guard is fold-invariant, CD defs (`_d` clone of
    `wantEdgeR_reconcileKeyDR_inert`). -/
theorem wantEdgeR_reconcileKeyDR_inert_d {σ : GraphState} {S : Schema} (T : Store)
    (dt on R : String) (e : Expr) (cands : List SubjectRef) (hRne : R ≠ BARE)
    (hσS : σ.schema = S)
    (hcands : ∀ c ∈ cands, c.predicate = BARE)
    (hRns : ∀ y, (objNode ⟨dt, on⟩ R, y) ∉ σ.edges)
...
/-- **The routed per-key edge characterisation, CD defs** (`_d` clone of
    `reconcileKeyDR_edge_char`; the guard-abstraction skeleton is identical, the
    fold-invariance transport is `wantEdgeR_reconcileKeyDR_inert_d`). -/
theorem reconcileKeyDR_edge_char_d {S : Schema} (T : Store) (dt on R : String) (e : Expr)
    (hRne : R ≠ BARE) (honStar : on ≠ STAR) (hder : isDerived S (dt, R) = true)
    (hcd : ComputedOrDirect e) (hba : DirectArmsBare e)
    (hrne : ∀ r' ∈ computedRefs e, r' ≠ R)
    (g : SubjectRef → Bool) :
  ...
  | cons c rest ih =>
    ...
    have hg1 : ∀ x ∈ rest, σc.wantEdgeR T dt on R e x = g x := by
      intro x hx
      rw [hσc, wantEdgeR_reconcileKeyDR_inert_d (S := S) T dt on R e [c] hRne hσS hcb1
        hRns honStar hder hcd hba hrne hcl x]
      exact hg x (List.mem_cons_of_mem _ hx)
    have hgc : σ.wantEdgeR T dt on R e c = g c := hg c List.mem_cons_self
    have hpairc : ∀ s : SubjectRef,
        ((subjNode s, objNode ⟨dt, on⟩ R) ∈ σc.edges
...
/-- **Pass-level routed edge exactness, CD defs** (`_d` clone of
    `reconcileStarsKeyDR_edge_char`). -/
theorem reconcileStarsKeyDR_edge_char_d {S : Schema} {σ : GraphState} (T : Store)
    (dt on R : String) (e : Expr) (shapes : List Shape)
    (cands negCands uposCands : List SubjectRef)
    (hσS : σ.schema = S) (hRne : R ≠ BARE) (honStar : on ≠ STAR)
  ...
  have hg : ∀ c ∈ cands, σr.wantEdgeR T dt on R e c
    ...
    have hcovr : σr.coveredAt (objNode ⟨dt, on⟩ R) R c.shape
      ...
      rfl
    rw [hchkr, hcovr]
  have hchar := reconcileKeyDR_edge_char_d (S := S) T dt on R e hRne honStar hder hcd hba
    hrne
    (fun c => σ.checkFnR T c dt on R e
      && !((shapes.filter (fun sh => σ.coveredFnR T dt on R e sh)).contains c.shape))
    cands σr hσrS hcb hRnsr hclr hg s
...
/-- **The routed batch edge origin at a fixed derived key, widened admission** (`_d`
    clone of `reconcileJobsLR_key_edge_sem`, over the FILTERED shadow): an edge of a
    routed logged batch at the key carries a `sem`-true subject or predates the
    batch. -/
theorem reconcileJobsLR_key_edge_sem_d {S : Schema} {T : Store} {σ0 : GraphState}
    (hWF : WF S) (hTT : TtuTuplesetsDirect S) (hNK : NodupKeys S) (hR : RewriteRanked S)
    ...
    (hRne : R ≠ BARE) (hon : on ≠ STAR)
    (hcd : ComputedOrDirect e) (hba : DirectArmsBare e)
    (hCOop_e : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
  ...
  | cons j rest ih =>
    ...
    have hσS' : (j.applyLoggedR S T σ).schema = S := by
      rw [W3cJob.applyLoggedR_schema, hσS]
    have hsh' : UntaintedShadow S (j.applyLoggedR S T σ) σ0 :=
      untaintedShadow_applyLoggedR_d hsh hunt hjv1
    have hRns' : ∀ y, (objNode ⟨dt, on⟩ R, y) ∉ (j.applyLoggedR S T σ).edges :=
      reconcileJobsLR_Rnode_not_source (jobs := [j]) hRne hjvs hRns
    have htb' : ∀ a b, (a, b) ∈ (j.applyLoggedR S T σ).edges → b.pred ≠ BARE :=
      reconcileJobsLR_target_ne_bare (jobs := [j]) hjvs htb
    ...
    · by_cases hkm : j.keyMatch dt on R
      · -- the pass targets the key: the routed char at pass start + the bridge
        ...
        have happlyedges : ((⟨dt, on, R, e, jc, jn, ju⟩ : W3cJob).applyLoggedR S T σ).edges
          ...
          rfl
        rw [happlyedges] at hmem
        have hchar := reconcileStarsKeyDR_edge_char_d (S := S) T dt on R e
          (wildcardShapes S) jc jn ju hσS hRne hon hder hcd hba hrne hcb hRns
          hsh.closed s
        rcases hchar.mp hmem with ⟨hcands, hguard⟩ | ⟨_, hold⟩
        · rw [Bool.and_eq_true] at hguard
          have hchk := hguard.1
          ...
          have hopsB : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
            ...
            intro x hxv y hxy
            exact htb y x hxy (hsbOps r' hr' hd' x hxv)
          rw [checkFnR_eq_sem_settled_d_filt hWF hTT hNK hR hSV hBS hTS hMatch hStrat
            hterm hcr hWSbare h0 hsh hσS hlk hder hcd hba hCOop_e hLU2e hopsB
            (fun hx => absurd hx hsstar) hon] at hchk
          exact Or.inl hchk
        · exact Or.inr hold
      · obtain ⟨_, hedges⟩ := applyLoggedR_other_key_fixed hjv1 hon hkm
...
/-- **Batch-level targeted re-settlement, widened admission** (`_d` clone of
    `settledComplete_jobsLR_targeted`, over the FILTERED shadow). -/
theorem settledComplete_jobsLR_targeted_d {S : Schema} {T : Store} {σ σ0 : GraphState}
    {jobs : List W3cJob}
    (hWF : WF S) (hTT : TtuTuplesetsDirect S) (hNK : NodupKeys S) (hR : RewriteRanked S)
    (hSV : StoreValidRulesD S T) (hBS : BareStarStore T) (hTS : TtuStarFree S T)
    (hMatch : RewriteMatchDeclared S) (hStrat : Stratifiable S)
  ...
  set σpre := reconcileJobsLR S T σ pre with hσpre_def
  have hshpre : UntaintedShadow S σpre σ0 :=
    untaintedShadow_reconcileJobsLR_d pre σ σ0 hsh hunt hjvpre
  have hσpreS : σpre.schema = S := by
    rw [hσpre_def, reconcileJobsLR_schema]
    exact hσS
  have hRnspre : ∀ y, (objNode ⟨dt, on⟩ R, y) ∉ σpre.edges :=
  ...
  have hbridge : ∀ (x : SubjectRef), (x.name = STAR → x.predicate = BARE) →
      σpre.checkFnR T x dt on R e = sem S T ⟨x, R, ⟨dt, on⟩⟩ :=
    fun x hx => checkFnR_eq_sem_settled_d_filt hWF hTT hNK hR hSV hBS hTS hMatch hStrat
      hterm hcr hWSbare h0 hshpre hσpreS hlk hder hcd hba hCOop_e hLU2e hopsB hx hon
  have hcovsem : ∀ sh ∈ declaredWildcardShapes S,
      σpre.coveredFnR T dt on R e sh = sem S T ⟨starSubj sh, R, ⟨dt, on⟩⟩ :=
    fun sh hws => hbridge (starSubj sh) (fun _ => hWSbare sh hws)
  -- …the Direct-arm / untainted-filtered twin of the same step.
  have hdeclOfCov : ∀ sh : Shape, σpre.coveredFnR T dt on R e sh = true →
      sh ∈ declaredWildcardShapes S := fun _ hcov =>
    checkFnR_star_declared_d_filt hTT hSV hTS h0 hshpre hσpreS hcr hNBD hlk hcd hba hon
      hopsB hcov
  -- the batch factors through the last targeting job
  have hfold : reconcileJobsLR S T σ jobs
      = reconcileJobsLR S T
  ...
  have happlyedges : ((⟨dt, on, R, e, jc, jn, ju⟩ : W3cJob).applyLoggedR S T σpre).edges
    ...
    rw [pushDelta_edges]
    rfl
  have hchar := reconcileStarsKeyDR_edge_char_d (S := S) T dt on R e (wildcardShapes S)
    jc jn ju hσpreS hRne hon hder hcd hba hrne hcb hRnspre hshpre.closed
  have hedgefinal : ∀ s : SubjectRef,
      ((subjNode s, objNode ⟨dt, on⟩ R) ∈ (reconcileJobsLR S T σ jobs).edges
        ↔ (subjNode s, objNode ⟨dt, on⟩ R)
            ∈ (σpre.reconcileStarsKeyDR T dt on R e (wildcardShapes S) jc jn ju).edges) := by
  ...
  · -- === the settled half ===
    ...
    · -- every derived edge witnesses a `sem`-true subject
      ...
      · rw [Bool.and_eq_true] at hguard
        ...
        rw [hbridge s (fun hx => absurd hx (hcS s hcands))] at hchk
        exact hchk
      · rcases reconcileJobsLR_key_edge_sem_d hWF hTT hNK hR hSV hBS hTS hMatch
            hStrat hterm hcr hNBD hWSbare h0 hlk hder hRne hon hcd hba hCOop_e hLU2e pre σ hσS
            hjvpre hsh hRns htb hsbOps hopsS hjsOpsPre s holdpre
          with hsem | hpreleg
        · exact hsem
        · exact absurd (hcovE s hpreleg) hncand
  · -- === the completeness half ===
...
/-- **The two-round targeted re-settlement, widened admission** (`_d` clone of
    `settledComplete_cascade2_targeted`). -/
theorem settledComplete_cascade2_targeted_d {σ : GraphState} {S : Schema} {T : Store}
    {jobs1 jobs2 : List W3cJob}
    (hWF : WF S) (hTT : TtuTuplesetsDirect S) (hNK : NodupKeys S) (hR : RewriteRanked S)
    (hSV : StoreValidRulesD S T) (hBS : BareStarStore T) (hTS : TtuStarFree S T)
    (hMatch : RewriteMatchDeclared S) (hStrat : Stratifiable S)
  ...
  obtain ⟨σ0, h0, hsh⟩ :=
    reachedByW3d2_shadow_d h hNK hCD hDAB hSV hterm hWF hBS hQ hDR hLS hMatch hNBD hTT hTS
  have hND : ∀ t' ∈ T.filter (fun tp => !isDerived S (tp.object.type, tp.relation)),
      isDerived S (t'.object.type, t'.relation) = false := by
    intro t' ht'
    simpa using (List.mem_filter.mp ht').2
  ...
  have hσS : σ.schema = S := reachedByW3d2_schema h
  have htb : ∀ a b, (a, b) ∈ σ.edges → b.pred ≠ BARE :=
    reachedByW3d2_edge_target_ne_bare_d h hWF hDAB hSV
  have hRns : ∀ y, (objNode ⟨dt, on⟩ R, y) ∉ σ.edges :=
    reachedByW3d2_Rnode_not_source hterm hRne hNBD hder h
  have hsbOps : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ∀ x, (x, objNode ⟨dt, on⟩ r') ∈ σ.edges → x.pred = BARE := by
    intro r' hr' hd' x hx
    exact reachedByW3d2_Rnode_source_bare_d h hWF hd' hDAB hSV x hx
  -- the accept form (the reject branch is dead at two strata)
  have hacc := runCascade2_no_abort hterm hNBD hLU2 hjv1 hjv2 hscope2 h
  set mid := reconcileJobsLR S T σ jobs1 with hmid_def
  -- the stratum fence: round 2 never targets a derived operand key
  have hfence : ∀ j2 ∈ jobs2, ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ¬ j2.keyMatch dt on r' := by
    intro j2 hj2 r' hr' hd' hkm
    obtain ⟨e', hlk'⟩ := isDerived_declared hd'
  ...
  · -- ==== Case A: settle over round 2 from the MID state ====
    have hshmid : UntaintedShadow S mid σ0 :=
      untaintedShadow_reconcileJobsLR_d jobs1 σ σ0 hsh hunt hjv1
    have hσmidS : mid.schema = S := by
      rw [hmid_def, reconcileJobsLR_schema]
      exact hσS
    have hRnsmid : ∀ y, (objNode ⟨dt, on⟩ R, y) ∉ mid.edges :=
    ...
    have hopsMid : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      · -- targeted in round 1 ⇒ re-settled by the batch lemma at σ (hops vacuous)
        ...
        have hLU2e' : ∀ r'' ∈ computedRefs e', isDerived S (dt, r'') = true →
          ...
          intro r'' hr'' hd'' _ _ _ _
          cases (hLUe' r'' hr'').symm.trans hd''
        refine settledComplete_jobsLR_targeted_d hWF hTT hNK hR hSV hBS hTS hMatch
          hStrat hterm hcr hNBD hWSbare h0 hsh hσS hjv1
          (covg_of_opsSettled hjv1 hcovg1 hlk'
            (fun r'' hr'' hd'' => by cases (hLUe' r'' hr'').symm.trans hd''))
          hlk' hd' hon
      ...
      · -- untargeted in round 1 ⇒ clean at leg start, settled at σ, transported
        ...
        · have hnot1 : ∀ j' ∈ jobs1, ¬ j'.keyMatch dt on r' :=
          ...
          exact ⟨settledKey_jobsLR_untargeted hjv1 hnot1 hon hset,
            completeKey_jobsLR_untargeted hjv1 hnot1 hon hcomp⟩
    have hsettled := settledComplete_jobsLR_targeted_d hWF hTT hNK hR hSV hBS hTS
      hMatch hStrat hterm hcr hNBD hWSbare h0 hshmid hσmidS hjv2
      (covg_of_opsSettled hjv2 hcovg2 hlk hopsMid) hlk hder hon hcd hba hCOop_e hLU2e
      hRnsmid htbmid hsbOpsmid hopsMid hfence hA
    rw [hacc]
    exact ⟨settledKey_congr rfl rfl hsettled.1, completeKey_congr rfl rfl hsettled.2⟩
  · -- ==== Case B: the last targeting job is in round 1; round 2 is inert here ====
    ...
    have hopsS : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      · exfalso
        ...
        exact hopsNoTgt j1 hj1 r' hr' hd' ⟨h1, h3, h2⟩
      · exact hsc
    have hsettled1 := settledComplete_jobsLR_targeted_d hWF hTT hNK hR hSV hBS hTS
      hMatch hStrat hterm hcr hNBD hWSbare h0 hsh hσS hjv1
      (covg_of_opsSettled hjv1 hcovg1 hlk hopsS) hlk hder hon hcd hba hCOop_e hLU2e
      hRns htb hsbOps hopsS hopsNoTgt htgtB
    have hnot2 : ∀ j ∈ jobs2, ¬ j.keyMatch dt on R :=
      fun j hj hkm => hA ⟨j, hj, hkm⟩
    rw [hacc]
    exact ⟨settledKey_congr rfl rfl
...
/-- **The stratum-staged settledness invariant, widened admission**
    (`reachedByW3d2C_settled_d`) — the three-disjunct form at every `ReachedByW3d2C`
    state over the Direct-arm fragment: schema-wide `ComputedOrDirect ∧
    DirectArmsBare` derived defs (derived OPERAND defs `ComputedOnly`), the store
    admitted by `StoreValidRulesD`. `hNoUD` (`exprDirects e = []` on derived defs —
    `Direct` arms only under `inter`/`excl`) scopes the `remove` leg: see
    `removeLeg_sem_stable2_d`'s header. -/
theorem reachedByW3d2C_settled_d {σ : GraphState} {S : Schema} {T : Store}
    (h : ReachedByW3d2C σ S T) :
    WF S → TtuTuplesetsDirect S → NodupKeys S → RewriteRanked S →
    RewriteMatchDeclared S → Stratifiable S →
    TtuTargetsSat S NotLeafName → DirectRestrictionsNotLeaf S →
  ...
  induction h with
  | empty S =>
    intro _hWF _hTT _hNK _hR _hMatch _hStrat _hQ _hDR _hLS _hcr _hNBD _hCD _hDAB _hCOop _hLU2 _hWSbare _hNoUD
    ...
    · intro s _ _ hedge
      simp [emptyState] at hedge
    · intro sh _ hsm
      have := sem_nil_false S ⟨starSubj sh, R, ⟨dt, on⟩⟩
      rw [hsm] at this
      exact absurd this (by decide)
    · intro s _ _ hsm _
      have := sem_nil_false S ⟨s, R, ⟨dt, on⟩⟩
      rw [hsm] at this
      exact absurd this (by decide)
    · intro s _ _ hsm
      have := sem_nil_false S ⟨s, R, ⟨dt, on⟩⟩
      rw [hsm] at this
      exact absurd this (by decide)
    · intro s _ _ hsemStar _
      have := sem_nil_false S ⟨starSubj s.shape, R, ⟨dt, on⟩⟩
      rw [hsemStar] at this
      exact absurd this (by decide)
  | @write σp S T t hadm hprev ih =>
    intro hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSV hBS hTS
    ...
    have hW3d2 : ReachedByW3d2 σp S T := reachedByW3d2C_toW3d2 hprev
    have hclp := reachedByW3d2_edgesClosed hW3d2
    rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSVw hBSw
        hTSw htermw dt on R e hlk hder hon with hdirty | hopdirty | ⟨hset, hcomp⟩
    · exact absurd (cascadeKeys_writeLeg_mono hclp _ hdirty) hmap
    · obtain ⟨r', hr', hd', hdirty'⟩ := hopdirty
      exact absurd ⟨r', hr', hd', cascadeKeys_writeLeg_mono hclp _ hdirty'⟩ hopmap
    · -- settled at σp, key and every derived operand key unmapped
      have hopsSettled : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
          SettledKey S T σp dt on r' ∧ CompleteKey S T σp dt on r' := by
        intro r' hr' hd'
        obtain ⟨e', hlk'⟩ := isDerived_declared hd'
        rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSVw
      ...
      have hsem : ∀ x : SubjectRef, (x.name = STAR → x.predicate = BARE) →
          sem S (t :: T) ⟨x, R, ⟨dt, on⟩⟩ = sem S T ⟨x, R, ⟨dt, on⟩⟩ :=
        fun x hx => writeLeg_sem_stable2_d hWF hTT hNK hR hSV hBS hTS hMatch hStrat hQ hDR hLS hcr
          hNBD hterm hCD hDAB hCOop hLU2 hWSbare hW3d2 hadm hlk hder hmap hopsUnmapped
          hopsSettled hx hon
      exact Or.inr (Or.inr
        ⟨settledKey_writeLeg_sem_d hWF hNK hadm honT hWSbare hder hon hmap hsem hset,
          completeKey_writeLeg_sem_d hWF hNK hadm honT hWSbare hder hon hmap hsem hcomp⟩)
  | @remove σp S T t hadm hdrain hSVT hBST hTST htermT hprev ih =>
    intro hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD _hSV _hBS _hTS
    ...
    by_cases hopmap : ∃ r' ∈ computedRefs e, isDerived S (dt, r') = true ∧
        (dt, r', on) ∈ cascadeKeys S (σp.removeLoggedRules S t)
    · exact Or.inr (Or.inl hopmap)
    have hSVDp : StoreValidRulesD S T :=
      storeValidRulesD_of_storeValidRules_directArmsBare hSVT hDAB
    have honT : t.object.name ≠ STAR := (hBST t hadm).2
    have hW3d2 : ReachedByW3d2 σp S T := reachedByW3d2C_toW3d2 hprev
    rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSVDp hBST
    ...
    · have hopsSettled : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
        ...
        intro r' hr' hd'
        obtain ⟨e', hlk'⟩ := isDerived_declared hd'
        rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSVDp
      ...
      have hsem : ∀ x : SubjectRef, (x.name = STAR → x.predicate = BARE) →
          sem S (T.erase t) ⟨x, R, ⟨dt, on⟩⟩ = sem S T ⟨x, R, ⟨dt, on⟩⟩ :=
        fun x hx => removeLeg_sem_stable2_d hWF hTT hNK hR hSVT hBST hTST hMatch hStrat hQ hDR hLS hcr
          hNBD htermT hCD hDAB hCOop hLU2 hWSbare hNoUD hW3d2 hadm hdrain hlk hder hmap
          hopsUnmapped hopsSettled hx hon
      exact Or.inr (Or.inr
        ⟨settledKey_removeLeg_sem_d hWF hNK honT hWSbare hder hon hmap hsem hset,
          completeKey_removeLeg_sem_d hWF hNK honT hWSbare hder hon hmap hsem hcomp⟩)
  | @cascade σp S T jobs1 jobs2 hjv1 hjv2 hcover1 hscope1 hcover2 hscope2 hcovg1 hcovg2
      hprev ih =>
    intro hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSV hBS hTS
      hterm dt on R e hlk hder hon
    ...
    · -- targeted ⇒ re-settled by the two-round settle theorem
      have hopsBase : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
        ...
        intro r' hr' hd'
        obtain ⟨e', hlk'⟩ := isDerived_declared hd'
        rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSV
      ...
      exact Or.inr (Or.inr (settledComplete_cascade2_targeted_d hWF hTT hNK hR hSV hBS
        hTS hMatch hStrat hQ hDR hLS hcr hNBD hterm hCD hDAB hCOop hLU2 hWSbare hW3d2 hjv1 hjv2 hcover1
    ...
    · -- untargeted: both dirty disjuncts force a targeting job; settled transports
      ...
      have hnot2 : ∀ j ∈ jobs2, ¬ j.keyMatch dt on R :=
        fun j hj hkm => htgt ⟨j, List.mem_append_right _ hj, hkm⟩
      rcases ih hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop hLU2 hWSbare hNoUD hSV hBS
...
/-- **T2b, W3d-2 Direct-arm fragment (`graph_correct_w3d2_d`) — `check = sem` at
    every fully-drained state of the TWO-STRATUM interleaved scheduler chain over
    `StoreValidRulesD` stores with `ComputedOrDirect ∧ DirectArmsBare` derived defs**
    (derived OPERAND defs `ComputedOnly`; `hNoUD` scopes the `remove` leg — see
    `removeLeg_sem_stable2_d`). The Direct-arm seed write's own-key dirtiness (the
    2026-07-20c `affectedKeys` model fix) is what makes the drained state settle its
    excluded seeds — the 2026-07-20b KILL state now DRAINS to `check = sem`,
    attack-confirmed by `#eval` before this proof was written. -/
theorem graph_correct_w3d2_d {S : Schema} {T : Store} {σ : GraphState} (q : Query)
    (hWF : WF S) (hTT : TtuTuplesetsDirect S) (hNK : NodupKeys S)
  ...
  have hschema : σ.schema = S := reachedByW3d2_schema hW3d2
  obtain ⟨σ0, h0, hsh⟩ :=
    reachedByW3d2_shadow_d hW3d2 hNK hCD hDAB hSV hterm hWF hBS hQ hDR hLS hMatch hNBD hTT hTS
  obtain ⟨⟨st, sn, sp⟩, R, ⟨dt, on⟩⟩ := q
  replace hqs : sn = STAR → sp = BARE := hqs
  replace hqo : on ≠ STAR := hqo
  -- `obtain` cased on `q`, so the query guard returns in projection form.
  ...
  · -- ===== derived query: the settled-key read =====
    ...
    have hCOop_e := hCOop _ _ _ hlk hder
    have hLU2e := hLU2 dt R e hlk hder
    -- the invariant at the drained state: every declared derived key settled+complete
    have hsettledAt : ∀ dt₀ on₀ R₀ e₀, S.lookup (dt₀, R₀) = some e₀ →
        isDerived S (dt₀, R₀) = true → on₀ ≠ STAR →
        SettledKey S T σ dt₀ on₀ R₀ ∧ CompleteKey S T σ dt₀ on₀ R₀ := by
      intro dt₀ on₀ R₀ e₀ hlk₀ hder₀ hon₀
      rcases reachedByW3d2C_settled_d h hWF hTT hNK hR hMatch hStrat hQ hDR hLS hcr hNBD hCD hDAB hCOop
    ...
    have hops : ∀ r' ∈ computedRefs e, isDerived S (dt, r') = true →
      ...
      obtain ⟨hset', hcomp'⟩ := hsettledAt dt on r' e' hlk' hd' hqo
      exact ⟨hset', hcomp',
        fun u hu => reachedByW3d2_reach_collapse_root_d hWF hDAB hSV hd' hW3d2 hu⟩
    -- no ghost star coverage — at ANY stratum, via the drained-state routed bridge
    -- (the T↾U hypothesis pack for the untainted-leaf star-declaredness)
    have hSVU : StoreValidRules S
        (T.filter (fun tp => !isDerived S (tp.object.type, tp.relation))) :=
      storeValidRules_untaintedFilter hSV
    have hStoreUntU : ∀ t ∈ T.filter
    ...
    have hsem_ws : ∀ sh : Shape, sh.2 = BARE →
      ...
      have hchk : σ.checkFnR T (starSubj sh) dt on R e = true := by
        rw [checkFnR_eq_sem_settled_d_filt hWF hTT hNK hR hSV hBS hTS hMatch hStrat
          hterm hcr hWSbare h0 hsh hschema hlk hder hcd hba hCOop_e hLU2e hops
          (fun _ => hshb) hqo]
        exact hsm
      unfold GraphState.checkFnR at hchk
      rcases evalE_computedOrDirect_true_leaf e hcd hchk with ⟨r', hr', hleaf⟩ |
        ⟨rs, hrs, hdl⟩
      · unfold GraphModel.graphRecR at hleaf
        cases hd' : isDerived S (dt, r') with
        | false =>
    ...
    have hroute : GraphModel.check σ ⟨⟨st, sn, sp⟩, R, ⟨dt, on⟩⟩
      ...
      GraphModel.check_derived σ _ (by rw [hschema]; exact hder)
    rw [hroute]
    exact probeDerived_eq_sem_settled hWSbare hsh.closed
      (fun u hu => reachedByW3d2_reach_collapse_root_d hWF hDAB hSV hder hW3d2 hu)
      hsem_ws hset hcomp hqs hqo
  · -- ===== untainted query: the filtered shadow + the untainted base equation =====
    have hd : isDerived S (dt, R) = false := by simpa using hder
    ...
    calc GraphModel.probeNonDerived σ ⟨⟨st, sn, sp⟩, R, ⟨dt, on⟩⟩
      ...
      _ = sem S (T.filter (fun tp => !isDerived S (tp.object.type, tp.relation)))
          ...
          graphRec_base_eq_bs_unt hWF hTT hNK hR hSVU hBSU hTSU hStoreUntU hMatch h0
            (s := ⟨st, sn, sp⟩) (dt := dt) (on := on) hqs hqo R hd
      _ = sem S T ⟨⟨st, sn, sp⟩, R, ⟨dt, on⟩⟩ :=
          (sem_untaintedFilter hNK hDecl hBS.noUsersetStar hTS
            ⟨⟨st, sn, sp⟩, R, ⟨dt, on⟩⟩ hd).symm
