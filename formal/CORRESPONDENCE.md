...
| **`GraphIndex/Leaf.lean::rawWriteRel` / `::rawWriteNode` / `::rawWriteTuple` / `::GraphState.writeDirectRaw`** (leg 7 step 4a; MAPPED 2026-08-14) | the RAW write re-addressed onto the leaf family — the TUPLE is forked, not the write path | `zanzibar_utils_v1.py::RuleSet.apply` re-addresses the triple via `::replace_relation`, then the ordinary write runs, so `GraphIndex/Write.lean::GraphState.writeDirect` stays byte-identical. ⚠ `rawWriteRel` models ONE undifferentiated leaf at index `0`; Python routes a Direct arm under `(viewer but not banned) or [user]` to `approver.2`, so any leaf→public map must be INDEX-AGNOSTIC (measured 2026-08-14) |
