...
* **`ResidueV1.version` is gated by nothing formal — now DECLARED as projection
  ...
  **The disposition is a MODELLING GAP, not a representation difference.**
  ...
  lean/ZanzibarProofs/GraphIndex/` returns only that comment. So unlike P1–P5
  (the other five, since P6 retired 2026-09-05),
...
* **★ ADJUDICATED 2026-07-29 — derived-edge MULTIPLICITY diverges: the model's
  ...
  **5c. Update 2026-09-05b (P3 landing) — TWO more list-vs-set gaps closed, and the ledger
  MOVED on every row.** The P3 flip doubled a derived-arm stacking that was already
  exponential in the number of adds (`two_stratum_cascade`, `alice → approver`: control
  `2, 13, 46, 204, 1013`, post-flip `4, 26, 92, 408, 2026`, and ten conformance tests hit
  `formal/conformance/runner.py`'s `_ZCLI_TIMEOUT_S`). Two places where the model kept a LIST that Python keeps as
  ...
  * `GraphIndex/CascadeStrataEnum.lean::enumJob2` / `::enumJob2D` wrap `cands` in
    ...
    one edge per candidate: that is the doubling. 5b's `freshDirectCands` filter is
    subsumed at the set level and kept only because `mem_enumJob2D_cands` /
    `w3dJobCoverage_enumJob2D` are stated against it; 5b's "filter DEFEATED" control
    (`golden=[16, 1] observed=[31, 1]`) is pre-dedup and would no longer move the ledger.
  Both are core `List.eraseDups` (first occurrence kept, Python dict order), NOT the
  Mathlib `.dedup` `rewriteClosure` uses below (last occurrence kept). Post-fix
  multiplicities `1, 2, 3, 4, 5` in 0.1 s; all 19 golden rows moved DOWN, control run first;
  `_MIN_LEDGER_STACKED` 19 → 18. Full record: `history/PROOF_STATUS.md` `2026-09-05b` §10.
  ...
  **6. Still open, deliberately.** The faithful model fix is to mirror Python's
  ...
  would catch it if one ever did. **Status 2026-09-05b: STILL OPEN**, but after 5c the
  residual is `+1` per reconcile of a key (linear in the number of rounds that dirty it),
  no longer a doubling per leg — `history/PROOF_STATUS.md` `2026-09-05b` §10.2.
  ...
  **Why not the 2026-07-29 narrow-the-projection move that worked for the derived
  ...
  rework.
  ★ **Re-pointed 2026-09-05 — that `generalize` line no longer reads that way, and
  the list-genericity it was cited for is exactly why it did not matter.** Since
  step R5 (the remove leg of THE FLIP) `GraphIndex/CascadeStrata.lean::count_removeLoggedRules`
  (`:716`) opens with `generalize rewriteClosureL S (rawWriteTuples S t) = us`
  (`:722`), because `GraphIndex/Cascade.lean::GraphState.removeLoggedRules` (`:340-341`)
  now folds the leaf-routed closure. `GraphIndex/CascadeStrata.lean::untOccCount` (`:687`)
  was restated over the same L closure. The 2026-08-08 sentence above is left as
  written (it was true of its tree); read it as an argument about the SHAPE of the
  proof — a `generalize` over whatever list the leg folds — not as a citation of
  current text.
  Narrowing again would also force the extractor to compute a
...
* **★★ NEW 2026-08-16b — the binary `Expr` CANNOT represent Python's leaf ALLOCATION
  for a parenthesized nested union, and no fix in `GraphIndex/Leaf.lean` can close it.**
  `Core/Schema.lean::Expr` models n-ary `union`/`inter` as BINARY nodes, and
  ...
  ```text
  r: a or b or safe      ->  2 leaves   (a@0, b@1; `safe` derived, consumes no index)
  r: (a or b) or safe    ->  1 leaf     (the inner PURE union merges)
  r: a or (b or safe)    ->  2 leaves
  ```

  `_fold_binary` maps the first two to the **same** `Expr`, so any model over this AST is
  faithful to at most one of them. `GraphIndex/Leaf.lean::persistedLeaves` models the
  **FLAT** form — the only form any corpus writes and the only one the encoder can
  round-trip — via `GraphIndex/Leaf.lean::unionSpineLeaves`, which flattens an impure
  union's left spine rather than merging it. The choice is stated as a theorem,
  `GraphIndex/Leaf.lean::LeafWitness.smN_models_the_flat_form`, rather than left in prose;
  `::LeafWitness.smN_nary_spine` pins the corrected allocation at the in-fragment corpus
  shape.
  **Why this is DECLARED rather than fixed:** making the model faithful to both requires an
  n-ary `Expr`, which is a `Core/` trust-root change touching every downstream proof — far
  outside leg 7's scope, and it would perturb the spec side that the leg is explicitly
  designed to spare.
  **Why a doc note is not enough, and what replaces it:** projection **P6** used to drop
  leaf edges, so the divergence was invisible and would first appear as a
  `diff_states` failure the moment leg 7 step 7 retired P6 — i.e. after the whole 4c-ii
  cone was paid. ★ **That happened 2026-09-05**: the cone is paid, P6 is retired, and leaf
  rows now reach the compare arm, so this divergence is a LIVE `diff_states` comparison
  rather than a hypothetical one. The shape is therefore refused MECHANICALLY at
  `formal/conformance/test_conformance_state.py::test_no_corpus_nests_a_pure_union_inside_an_impure_one`
  (sabotage-verified: rewriting `nary_union_derived4`'s `a or b or c or safe` as
  `(a or b) or c or safe` makes it fail, naming the offending key). A corpus can no longer
  acquire the shape silently.
  ⚠ **How this was FOUND is itself the lesson**, and it is recorded in
  `docs/sabotage-procedure.md`: the instrument that validated the two preceding allocation
  fixes ("82/82 derived keys agree") transcribed the Lean rule into Python but fed it
  ...
  (`GraphIndex/Leaf.lean::rawWriteRels` vs `zanzibar_utils_v1.py::RuleSet.apply`, 744/744),
...
* **~~No leaf-family split~~ — RESOLVED 2026-09-05** (leg 7 step 4c-ii + step 7, "the
  ...
  `<rel>.<i>` rows the body below describes are MODELLED rather than projected away, and
  projection **P6** is retired — the P6 cite at the end of this entry is HISTORICAL. The
  live pin at that position is the state gate's own comparison of those rows. Original
  body, 2026-07-12, kept verbatim: The model reads raw boolean defs (`ComputedOnly`
