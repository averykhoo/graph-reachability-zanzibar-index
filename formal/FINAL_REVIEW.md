...
3. **Fragment scope** (each a documented gap, none hidden — `history/ROADMAP.md`
   ...
   `test_conformance_remove.py`; the residual open slice is §4(d)'s: the Exec
   driver / zcli graph mode is still add-only as of 2026-07-19, so remove-
   correctness is PROVED but not yet end-to-end DRIVEN, and the guard's validly-
   stored precondition is flagged for review); star-subject queries with non-bare
...
never more. Remaining open slice: the Exec driver / zcli graph mode is still
add-only as of 2026-07-19 (it constructs no remove-state), so remove-correctness
is PROVED over the operational chain but not yet DRIVEN end-to-end — a flagged
follow-up — and the guard's validly-stored scope decision (it strengthens an
audited inductive) is flagged for user review;
