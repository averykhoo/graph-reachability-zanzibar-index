...
| [`../../formal/ARCHITECTURE.md`](../../formal/ARCHITECTURE.md) | the **Lean formal layer**: a machine-checked proof that both backend algorithms compute `sem` (hence are equivalent), pinned to Python by a conformance harness. Scope is asymmetric — set engine at full scope, graph index only at a documented fragment — which since 2026-08-05 includes `Direct`-arm boolean stores for every graph theorem **except T2a `graph_reached_inv`, still vacuous there**. The exact claim is [`../../formal/FINAL_REVIEW.md`](../../formal/FINAL_REVIEW.md) (§3.0 for the Direct-arm history), and nothing anywhere may claim more than it |
