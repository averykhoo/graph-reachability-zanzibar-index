...
| [`../../formal/ARCHITECTURE.md`](../../formal/ARCHITECTURE.md) | the **Lean formal layer**: a machine-checked proof that both backend algorithms compute `sem` (hence are equivalent), pinned to Python by a conformance harness. Scope is asymmetric — set engine at full scope, graph index only at a documented fragment, and **vacuously on `Direct`-arm boolean stores**. The exact claim is [`../../formal/FINAL_REVIEW.md`](../../formal/FINAL_REVIEW.md) (§3.0 for the vacuity), and nothing anywhere may claim more than it |
