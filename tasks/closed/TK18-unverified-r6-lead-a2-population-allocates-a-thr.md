...
`dict.get(k, self.ops.new())` evaluates its default EAGERLY, so every `population()` call constructs a fresh C-level `BitMap()` even on a hit. `population` is the `pop` callback threaded through the whole `MemberSet` algebra (`setengine/memberset.py::_starpop` calls it once per star shape inside every `_ext`/`_normalize`), i.e. several times per algebra step. The sketch is the two-line `m = get(k); return m if m is not None else new()`. Finder: category repeated-work, filed impact low, algorithm change no.
...
- [`docs/perf-round6-audit-2026-08.md`](docs/perf-round6-audit-2026-08.md)`:775` — this lead, verbatim (evidence + unreviewed fix sketch)
...
- `setengine/engine.py::SetEngine.population` — the code
