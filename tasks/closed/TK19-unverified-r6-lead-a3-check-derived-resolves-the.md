...
`index_v4/wildcard.py::WildcardIndex._check_derived` — lead **A3** of the 16-item appendix, `docs/perf-round6-audit-2026-08.md:787`.

In the `...`-subject branch, `_check_derived` resolves the object node for the edge probe and then, on a probe miss, calls `_residue_state`, whose body re-runs the identical point SELECT — outside a reconcile the `P3` `_residue_cache` is `None`, so every failed-probe derived check pays a duplicated node resolution. `_residue_state` also decodes all three JSON blobs and rebuilds fresh `set()`s even when the caller wants one O(1) membership answer, making a point check O(|neg|). Finder: repeated-work, medium, no.
...
⚠ **This lead and lead A14 (`:919`) are the same neighbourhood from two sides** — A3 removes the duplicate resolve and memoizes per call, A14 proposes a persistent version-keyed snapshot cache on `WildcardIndex`. Adjudicate them together or the second one lands on a moved target. Both key invalidation on `ResidueV1.version` (I7), which is modelled by nothing formal — see the separately filed I7 modelling-gap row.

## Read first

- [`docs/perf-round6-audit-2026-08.md`](docs/perf-round6-audit-2026-08.md)`:787` — this lead, verbatim (evidence + unreviewed fix sketch)
...
- `index_v4/wildcard.py::WildcardIndex._check_derived` — the code
...
**Provenance.** COVERAGE.md PART 1 U-17, the `R6-A1..R6-A16` block (tier 3, sweep-g only; sweep-l never reached the appendix). Source: docs/perf-round6-audit-2026-08.md:787, inside the appendix at :754-953. CONFIRMED OPEN AND UNCHANGED by COVERAGE.md §C3: `grep -c 'R6-A'` -> 0, no id anywhere. Lead 3 of 16.
